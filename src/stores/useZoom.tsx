...
const handleZoomIn = (get: StoreApi<IZoomStore>['getState']) => () =>
  get().setZoom(get().zoom * 1.1)

const handleZoomOut = (get: StoreApi<IZoomStore>['getState']) => () =>
  get().setZoom(get().zoom / 1.1)

const handleSetZoom = (set: StoreApi<IZoomStore>['setState']) => (zoom: number) =>
  set(() => {
...
const handleResetZoom = (set: StoreApi<IZoomStore>['setState']) => () => {
  set(() => {
    return { zoom: 1 }
  })
}
